...
then
	...
	then
		...
		bod; read -r -p "URL: " url; echo -e "${YELLOW}[-] Parsing Js...${RESTORE}\n";
		validate $url
	    ruby ~/relative-url-extractor/extract.rb $url; bod; sleep 1
	else
...
shodan_verify=$(curl -s curl "https://api.shodan.io/shodan/host/8.8.8.8?key=${shodan_api}" | grep -o "bad password")
